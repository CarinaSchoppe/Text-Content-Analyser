...
def convert_chat_gpt_answer(input_text: str, output: str):
    ...
    for answer in answers:
        ...
        try:
            ...
            break
    if valid:
        if input_text not in dict_answers:
            dict_answers[input_text] = {}
        for answer in valid_answers:
            first_entity, relation, second_entity = answer
            # get the len of elements in the dict_answers[input] and add 1 to it
            index = len(dict_answers[input_text])
            dict_answers[input_text][index] = (first_entity, relation, second_entity)
